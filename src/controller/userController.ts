...
import { calculateTxFee, delay, getBtcUtxoByAddress, getFeeRate, getRuneUtxoByAddress, } from '../service/service';
...
export const generateInitialRuneSwapPsbt = async (pubkey: string, userAddress: string, userOrdinalPubkey: string, userOrdinalAddress: string, sendingAmount: number, walletType: string) => {
  const mock = await getMockContent();
  ...
  const userRuneUtxos = await getRuneUtxoByAddress(userAddress, userRuneId);
  const adminRuneUtxos1 = await getRuneUtxoByAddress(adminWallet.address, adminRuneId1);
  const adminRuneUtxos2 = await getRuneUtxoByAddress(adminWallet.address, adminRuneId2);
